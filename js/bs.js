...
var defaults = {
  ...
  ne = {
    ...
    daysName: ['आइतबार', 'सोमबार', 'मंगलबार', 'बुधबार', 'बिहिबार', 'शुक्रबार', 'शनिबार'],
    daysShortName: ['आइत', 'सोम', 'मंगल', 'बुध', 'बिहि', 'शुक्र', 'शनि'],
    daysMinName: ['आ', 'स', 'म', 'ब', 'बि', 'शु', 'श']
  },
  en = {
    ...
    daysName: ['Aitabar', 'Somabar', 'Mangalbar', 'Budhabar', 'Bihibar', 'Shukrabar', 'Shanibar'],
    daysShortName: ['Ait', 'Som', 'Mang', 'Bud', 'Bih', 'Shuk', 'Shan'],
    daysMinName: ['A', 'S', 'M', 'B', 'B', 'S', 'S']
  },
...
function getNepaliNumber(number) {
    var nepaliNumber = '';
    var nepaliNumberArray = number.toString().split('');
    for (var i = 0; i < nepaliNumberArray.length; i++) {
        nepaliNumber += nums[nepaliNumberArray[i]];
    }
    return nepaliNumber;
}

// getNepDayOfWeek
function getNepDayOfWeek(day,config){
    let lang = config?.lang || 'ne';
    if(lang=="en"){
        return {
          full:en.daysName[day],
          short:en.daysShortName[day],
          min:en.daysMinName[day]
        }
    }else{
        return {
          full:ne.daysName[day],
          short:ne.daysShortName[day],
          min:ne.daysMinName[day]
        }
    }
}
